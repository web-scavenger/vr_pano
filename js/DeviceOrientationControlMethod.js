...
DeviceOrientationControlMethod.prototype._handleData = function (data) {
  ...
  if (previous.yaw != null && previous.pitch != null && previous.roll != null) {
    ...
    // console.log(this._dynamics.yaw)
    this.emit('parameterDynamics', 'yaw', this._dynamics.yaw);
    this.emit('parameterDynamics', 'pitch', this._dynamics.pitch);
  }
  // get hotlink class list and loop 
  
  var pointBlock = document.getElementById('js-point-block').style.display; // check point at the screen
  if (pointBlock != 'none') {
    
    var point = document.getElementById('js-point');
   
    var pointloc = point.getBoundingClientRect()

    //check pointer at all links in the room
    panoramicApp.opt.linksArr.forEach(function(element){
      var className = element.getRoomName() + element.checkLinks().target;
      var getEl = document.getElementsByClassName(className); //links position on the screen
      
      var rect = getEl[0].getBoundingClientRect();
      var nextRoom = element.checkLinks().target;
      element.changeRoom(rect, pointloc, nextRoom, getEl[0]);
    })
